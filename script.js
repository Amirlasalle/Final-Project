...
//# represents an id, and . represents a class//
//1st set of codes
/*const colors = [
    "#4f91cd",
    "#164f52",
...
function makeBall(color, ballSize, ballY, ballX) {
...
let numOfBalls = 0;
let limitOfNumberOfBalls = 100000;
const { button, input, container } = init();

button.addEventListener("click", () => {
    console.log(numOfBalls) 
    if (numOfBalls > limitOfNumberOfBalls) {
     } 
     else {
...
function makeBalls(numOfBalls, container) {
    for (let i = 0; i < parseInt(numOfBalls); i++) {
        const { randomColor, ballSize, ballX, ballY} = genParamas();
        console.log({randomColor, ballSize, ballX, ballY});
        container.append(makeBall(randomColor, ballSize, ballX, ballY));
    }
}
const colors = [
